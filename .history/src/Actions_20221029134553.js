...
export const getProjectDetails = (access_token) => {
  ...
  return async (dispatch) => {
    ...
    try {
      await axios
        .get(
          "http://52.139.224.15:81/my_project_list/?search=&page?limit=5&ordering=-id&project_status=&solution_org=",
          config
        )
        .then((response) => {
          console.log("response from api", response);
          if (response.status == 200) {
            dispatch({ type: PROCESS_SUCESS, payload: response.data.results });
          }
        });
    } catch (err) {
      console.log(err);
...
export const getProcesslines1 = (access_token, id) => {
  console.log(access_token);
  console.log(id);
  ...
  return async (dispatch) => {
    ...
    try {
      await axios
        .get(
          `http://52.139.224.15:81/end_user/process_line/?line_id=${id}`,
          config
        )
        .then((response) => {
          console.log("response from api", response);
          if (response.status == 200) {
            dispatch({ type: PROJECT_SUCESSC, payload: response });
          }
        });
    } catch (err) {
      console.log(err);
...
export const getEquipmentDetails1 = (access_token, id) => {
  console.log(access_token);
  console.log(id);
  return async (dispatch) => {
    let config = {
    ...
    try {
      await axios
        .get(
          `http://52.139.224.15:81/tagged_equipment_list/?installed_equipment_id=${id}`,
          config
        )
        .then((response) => {
          console.log("response from api", response);
          if (response.status == 200) {
            dispatch({ type: EQUIPMENT_SUCESSC, payload: response });
          }
        });
    } catch (err) {
      console.log(err);
